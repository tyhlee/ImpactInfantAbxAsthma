data {
  ...
  int<lower=0,upper=1> adj_rev_causation[N]; // indicator for adjustment for rev causation
  int<lower=1> N_test;
...
transformed parameters{
  ...
  for (i in 1:N){
    log_mu[i] = beta[1] + beta[2]*t[i] + beta[3]*dose[i]  + beta[4]*adj_rev_causation[i] +  u[studyid[i]];
  }
}
...
model {
  ...
  beta ~ multi_normal([0,0,0,0],Sigma);
  //likelihood
  for (i in 1:N){
...
generated quantities{
  ...
  for(j in 1:N_test){
    y_test[j] = normal_rng(beta[1] + beta[3]*x_test[j,2]+beta[2]*x_test[j,1]+beta[4],sigma_u);
  }
  
  for(k in 1:N_test){
    y_patrick[k] = beta[1] + beta[3]*x_test[k,2]+beta[2]*x_test[k,1]+beta[4]+u[studyid[1]];
  }
}
